\input{../componenti_comuni/src/commands.tex}
\documentclass[12pt, boldsubject, italicsignature, noindent]{letteracdp}
\usepackage{eurosym}
\usepackage{graphicx}
\usepackage{hyperref}
\hypersetup{
    colorlinks=true,
    linkcolor=black,
    urlcolor=blue
}

\renewcommand{\capName}{\textit{Trustify}} %Nome capitolato
\renewcommand{\capCode}{C7} %Numero CX
\renewcommand{\capLink}{https://www.math.unipd.it/~tullio/IS-1/2022/Progetto/C7.pdf} %Link pagina prof
\renewcommand{\capProposer}{SyncLab} % Nome azienda

\address{
    \begin{minipage}{0.49\textwidth}
        \begin{flushleft}
            \begin{minipage}{0.8\linewidth}
                \includegraphics[width=1\linewidth]{../componenti_comuni/img/logo_swemming.pdf}
            \end{minipage}
        \end{flushleft}
    \end{minipage}
    \begin{minipage}{0.49\textwidth}
        \begin{flushright}
            \begin{minipage}{0.8\linewidth}
                \includegraphics[width=0.65\linewidth]{../componenti_comuni/img/logo_unipd.pdf}  
            \end{minipage}
        \end{flushright}
    \end{minipage}
    
    Gruppo 9, SWEmming Pool, \\
    Corso Ingegneria del Software \\
    A.A. 2022/2023 \\
    E-mail: \groupMail
    }    
\signature{SWEmming Pool}
% \date{27 Ottobre 2022}
\date{}

\begin{document}

\begin{letter}{
    \Vardanega \\
    \Cardin \\
    Dipartimento di Matematica \\
    Università degli Studi di Padova \\
    Via Trieste, 63 \\ 35121 Padova
}

\opening{Egregio \vardanega,\\ Egregio \cardin}

Con la presente il gruppo \groupName intende candidarsi ufficialmente per la realizzazione del prodotto \capName (capitolato \capCode) da Voi commissionato e proposto dall'azienda \capProposer.

Tutti i documenti prodotti fino a questo punto sono raccolti nel repository presente all'indirizzo 
\begin{center}
    \url{https://github.com/SWEmming-Pool/documentazione}
\end{center}

Inoltre, come descritto nel documento \textit{"Preventivo di progetto"} presente nel repository della documentazione, i costi preventivati per lo sviluppo del prodotto sono pari a \euro{}$X\ge  10.285,714286$ e consegna stimata entro il giorno \textit{giorno-mese-anno}. 

\pagebreak
Di seguito si riporta la lista dei componenti del gruppo:

\begin{center}
    \begin{tabular}{l|c}
    \textbf{Nominativo} & \textbf{Matricola} \\
    Enrico Bacci Bonivento & 1101378 \\
	Ennio Italiano & 1224819 \\
	Fabio Pantaleo & 2009124 \\     
	Elia Pasquali & 1225412 \\	     
	Sebastiano Sanson & 2011880 \\
    Nicolò Trinca & 2011070
   \end{tabular}
\end{center}

\closing{Cordiali saluti,}

\end{letter}

\end{document}